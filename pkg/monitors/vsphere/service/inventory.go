...
func NewInventorySvc(gateway IGateway) *InventorySvc {
	return &InventorySvc{gateway: gateway}
}
...
// Retrieves the contents of the passed-in folder and puts them in the passed-in Inventory object.
func (svc *InventorySvc) retrieveFolderContents(folder *mo.Folder, inv *model.Inventory) (*model.Inventory, error) {
	...
	for _, ref := range folder.ChildEntity {
		...
		default:
			Log.WithField("ref", ref).Warn("retrieveFolderContents: unknown type")
		}
	}
	return inv, nil
}
...
// Retrieves the contents of the passed-in cluster and puts them in the passed-in Inventory object.
