...
func NewMetricsService(gateway IGateway) *MetricsSvc {
	return &MetricsSvc{gateway: gateway}
}
...
func (svc *MetricsSvc) RetrievePerfCounterIndex() (model.MetricInfosByKey, error) {
	...
	if err != nil {
		Log.WithError(err).Error("retrievePerformanceManager failed")
		return nil, err
	}
...
func (svc *MetricsSvc) PopulateInvMetrics(inv *model.Inventory) {
	for _, invObj := range inv.Objects {
		...
		if err != nil {
			Log.WithField("invObj.ref", invObj.Ref).WithError(err).Error("populateInvMetrics: queryAvailablePerfMetric failed")
			continue
		}
